...
static int gaussint_fit(void * vstate);
...
#if 0
  {
    gsl_matrix *ATA = gsl_matrix_alloc(state->p, state->p);
    gsl_vector *ATb = gsl_vector_alloc(state->p);
    gsl_vector *tmp = gsl_vector_alloc(state->p);
    double normb = gsl_blas_dnrm2(&b.vector);
    double xTATb, xTATAx, r2, rnorm2;

    gsl_blas_dsyrk(CblasLower, CblasTrans, 1.0, &A.matrix, 0.0, ATA);
    gsl_blas_dgemv(CblasTrans, 1.0, &A.matrix, &b.vector, 0.0, ATb);

    gsl_blas_ddot(state->c, ATb, &xTATb);

    gsl_blas_dsymv(CblasLower, 1.0, ATA, state->c, 0.0, tmp);
    gsl_blas_ddot(state->c, tmp, &xTATAx);

    r2 = normb*normb - 2.0*xTATb + xTATAx;
    fprintf(stderr, "\t r2 = %g\n", r2);
  }
#endif
