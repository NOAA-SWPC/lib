...
secs_read(const char *filename)
{
  ...
  if (!fp)
    {
      fprintf(stderr, "secs_read: unable to open %s: %s\n",
              filename, strerror(errno));
      return NULL;
    }

  data = calloc(1, sizeof(secs_data));
...
static size_t
find_pairs_mag(const double longitude, const secs_data *secs, const grobs_data *mag,
  ...
  for (i = 0; i < secs->n; ++i)
    {
      time_t t = secs->t[i];
      double dt, dH;
      double londiff = wrap180(secs->longitude[i] - longitude);
      double kp;
...
main(int argc, char *argv[])
{
  ...
  if (!data_wamnet)
    {
      fprintf(stderr, "Usage: %s <-i secs_current_file> [-o output_file] [-w wamnet_file]\n", argv[0]);
      exit(1);
    }
  ...
  if (data_wamnet)
    {
      ...
      size_t npairs = find_pairs_mag(data_wamnet->glon, data_secs, data_wamnet,
                                     dH, J);
