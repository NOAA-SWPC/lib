      ...
      for (phi = 0.0; phi <= 2.0 * M_PI; phi += dphi)
        {
          ...
          secs1d_df_green(r, theta, theta0, B, w);
    ...
    for (theta = eps; theta <= M_PI - eps; theta += dtheta)
      {
        ...
        secs1d_df_green_J(theta, theta0, K, w);
...
main()
{
  const size_t npoles = 200;
  secs1d_workspace *w;

  /*w = secs1d_alloc(SECS1D_LMAX, R_EARTH_KM + 110.0, npoles);*/
  w = secs1d_alloc(100, R_EARTH_KM + 110.0, npoles);
