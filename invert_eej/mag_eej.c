...
static int gsl_multifit_linear_lcorner3(const gsl_vector *rho,
...
/*
points2circle()
  Find the circle which passes through 3 given points
(x1,y1), (x2,y2), (x3,y3). Return location of
circle's center in (a,b) and circle radius in r

Return: GSL_SUCCESS upon success; GSL_EINVAL if points
        are colinear
*/

static int
points2circle(const double x1, const double y1,
              ...
              double *a, double *b, double *r)
{
  ...
  const double h21 = x21*x21 + y21*y21;
  const double h31 = x31*x31 + y31*y31;
  const double d = 2.0 * (x21*y31 - x31*y21);
  double xc, yc, radius;

  /* center of circle */
  xc = x1 + (h21 * y31 - h31 * y21) / d;
  yc = y1 - (h21 * x31 - h31 * x21) / d;

  /* circle radius */
  radius = sqrt(h21*h31*((x3-x2)*(x3-x2)+(y3-y2)*(y3-y2))) / fabs(d);

  if (!gsl_finite(xc) || !gsl_finite(yc) || !gsl_finite(radius))
    return GSL_EINVAL;
...
#if 0
static int
gsl_multifit_linear_lcorner3(const gsl_vector *rho,
                             const gsl_vector *eta,
                             size_t *idx)
{
      ...
      for (i = 1; i < n - 1; ++i)
        {
          ...
          s = points2circle(x1, y1, x2, y2, x3, y3, &a, &b, &r);
          if (s == GSL_SUCCESS)
            {
              /*
               * check for smaller radius of curvature; and
               * check that circle center is above L-curve
               */
              if ((r < rmin || rmin < 0.0) &&
                  (a >= x2 && b >= y2))
                {
