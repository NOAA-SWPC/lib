...
static void tcalc_get_date(double fday, int *year, int *month, double *day);

static int months[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

/*
tcalc_sunrise()
  Compute local time of sunrise on 'fday' (in minutes)

Inputs: fday - number of days since Jan 1, 2000
        lat  - latitude in radians
        long - longitude in radians
*/

double
tcalc_sunrise(double fday, double lat, double lon)

{
  ...
  tcalc_get_date(fday, &year, &month, &day);
...
/*
tcalc_sunset()
  Compute local time of sunset on 'fday' (in minutes)
...
double
tcalc_sunset(double fday, double lat, double lon)
...
static void
tcalc_get_date(double fday, int *year, int *month, double *day)

{
  int sum;
  int i;

  *year = (int) (fday / 365.25 + 2000.0);
  *month = *day = 1;

  /* get day in range 0-365.25 */
  while (fday > 365.25)
    fday -= 365.25;
  while (fday < 0.0)
    fday += 365.25;
  ...
  for (i = 0; i < 12; ++i)
    {
      sum += months[i];
      if (i == 1 && tcalc_is_leap_year(*year))
        ++sum;
